...
namespace MemberDetection
{
    class ListPoints
    {
        ...
        public Dictionary<Vector2, double> removedPointOnParallelLines(List<double> angles)
        {
            ...
            {
                if (angles[i] >= 0.1 && angles[i] <= 179.9)
                {
        ...
        {
            Dictionary<List<Vector3>, double> dictionary = new Dictionary<List<Vector3>, double>();
            for (int i = 0; i < Point3Ds.Count - 1; i++)
            {
                for (int j = i + 1; j < Point3Ds.Count; j++)
                {
                    ...
                    dictionary.Add(list, distance);
                }
            }

            var maxDistanceKey = dictionary.OrderByDescending(x => x.Value).FirstOrDefault().Key;
